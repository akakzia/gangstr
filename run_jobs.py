...
nb_seeds = 1
values = [0.05, 0.02, 0.01, 0.005, 0.002, 0.001]

for i in range(nb_seeds):
    for value in values:
        job_file = os.path.join(job_directory, "rebuttal_{}%.slurm".format(value*100))

        with open(job_file, 'w') as fh:
            fh.writelines("#!/bin/bash\n")
            fh.writelines("#SBATCH --account=oke@cpu\n")
            fh.writelines("#SBATCH --job-name=inter_{}\n".format(value))
            fh.writelines("#SBATCH --partition=cpu_p1\n")
            fh.writelines("#SBATCH --qos=qos_cpu-t4\n")
            fh.writelines("#SBATCH --output=rebuttal_{}%_%j.out\n".format(100*value))
            fh.writelines("#SBATCH --error=rebuttal_{}%_%j.out\n".format(100*value))
            ...
            fh.writelines("srun python -u -B train.py --internalization-prob 1.0 --ss-internalization-prob 0.5 --add-all-permutations --intervention-prob {} --save-dir 'rebuttal_{}%/' 2>&1 ".format(value, value*100))
